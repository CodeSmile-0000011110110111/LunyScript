...
namespace LunyScript
{
	...
	{
		private const Boolean CollectCallStackInfo = true;
		...
		public ComponentApi Component => new(this, TryCreateStackTrace(nameof(Component)));
		public DebugApi Debug => new(this, TryCreateStackTrace(nameof(Debug)));
		public EditorApi Editor => new(this, TryCreateStackTrace(nameof(Editor)));
		public InputBuilder Input => new(this, TryCreateStackTrace(nameof(Input)));
		public ObjectBuilder Object => new(this, TryCreateStackTrace(nameof(Object)));
		public OnObjectEventBuilder On => new(this, TryCreateStackTrace(nameof(On)));
		public PrefabBuilder Prefab => new(this, TryCreateStackTrace(nameof(Prefab)));
		public SceneApi Scene => new(this, TryCreateStackTrace(nameof(Scene)));
		public TimeApi Time => new(this, TryCreateStackTrace(nameof(Time)));
		public TransformBuilder Transform => new(this, TryCreateStackTrace(nameof(Transform)));
		public WhenGlobalEventBuilder When => new(this, TryCreateStackTrace(nameof(When)));

		private static StackTrace TryCreateStackTrace(String apiName) =>
			CollectCallStackInfo ? new StackTrace(apiName, new StackFrame(2, true)) : null;
 	...
 	/// <summary>
		...
		/// Multiple conditions are implicitly AND combined.
		/// </summary>
		public WhileBlockBuilder While(params ConditionBlock[] conditions) => new(conditions, TryCreateStackTrace(nameof(While)));

		/// <summary>
		/// For loop (1-based index): For(numberOfTimes).Do(blocks);
		/// Starts at 1 and increments by 1 until limit is reached (inclusive).
		/// </summary>
		public ForBlockBuilder For(VariableBlock numberOfTimes) => new(numberOfTimes, TryCreateStackTrace(nameof(For)));
		...
		/// </summary>
		public ForBlockBuilder For(VariableBlock numberOfTimes, VariableBlock step) => new(numberOfTimes, step, TryCreateStackTrace(nameof(For)));
		...
		/// <returns></returns>
		public ConditionBlock Check(Func<IScriptRuntimeContext, Boolean> func) => CheckBlock.Create(func, TryCreateStackTrace(nameof(Check)));
		...
		/// <param name="func"></param>
		/// <returns></returns>
		public ConditionBlock Check(Func<Boolean> func) => CheckBlock.Create(_ => func(), TryCreateStackTrace(nameof(Check)));
		...
		/// <returns></returns>
		public ActionBlock Run(Action action) => RunBlock.Create(_ => action(), TryCreateStackTrace(nameof(Run)));
		...
		/// <param name="action"></param>
		/// <returns></returns>
		public ActionBlock Run(Action<IScriptRuntimeContext> action) => RunBlock.Create(action, TryCreateStackTrace(nameof(Run)));

		/// <summary>
		/// Logical AND: Returns true if all conditions are true. Requires at least two conditions.
		/// </summary>
		public ConditionBlock AND(params ConditionBlock[] conditions) => AndOperatorBlock.Create(conditions, TryCreateStackTrace(nameof(AND)));

		/// <summary>
		/// Logical OR: Returns true if at least one condition is true. Requires at least two conditions.
		/// </summary>
		public ConditionBlock OR(params ConditionBlock[] conditions) => OrOperatorBlock.Create(conditions, TryCreateStackTrace(nameof(OR)));

		/// <summary>
		/// Logical NOT: Returns the inverse of the condition.
		/// </summary>
		public ConditionBlock NOT(ConditionBlock condition) => NegationOperatorBlock.Create(condition, TryCreateStackTrace(nameof(NOT)));
	}
}
