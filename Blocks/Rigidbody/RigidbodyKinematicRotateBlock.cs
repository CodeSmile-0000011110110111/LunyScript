...
namespace LunyScript.Blocks
{
	internal sealed class RigidbodyKinematicRotateBlock : ActionBlock
	{
		...
		internal static RigidbodyKinematicRotateBlock CreateAxisRelative(VariableBlock amount, LunyAxis axis, LunyTransformSpace space, StackTrace trace) =>
		...
		internal static RigidbodyKinematicRotateBlock CreateVector(LunyVector3 eulerDelta, LunyTransformSpace space, StackTrace trace) =>
			new(null, default, eulerDelta, useVector: true, space, trace);

		private RigidbodyKinematicRotateBlock(VariableBlock amount, LunyAxis axis, LunyVector3 eulerDelta, Boolean useVector, LunyTransformSpace space, StackTrace trace)
			: base(trace)
		{
			_amount = amount;
			_axis = axis;
		...
		protected internal override void Execute(IScriptRuntimeContext context)
		{
			...
			if (rigidbody == null)
			{
				LunyLogger.LogWarning($"{nameof(RigidbodyKinematicRotateBlock)}: no {nameof(ILunyRigidbody)} on '{context.LunyObject.Name}'", context.LunyObject);
				return;
			}
